 ...
 *
 * $Id: pcapng.c 28739 2009-06-15 15:24:40Z gerald $
 *
 * Wiretap Library
...
pcapng_read_packet_block(FILE_T fh, pcapng_block_header_t *bh, pcapng_t *pn, wtapng_block_t *wblock,int *err, gchar **err_info _U_, gboolean enhanced)
{
	...
	errno = WTAP_ERR_CANT_READ;
	if (enhanced) {
		bytes_read = file_read(&epb, 1, sizeof epb, fh);					// BUG_8B973D84(1) FIX_8B973D84(1) BUG_3E6EE6AF(1) FIX_3E6EE6AF(1) #Read the header from the file
		...
		if (pn->byte_swapped) {
			...
			wblock->data.packet.cap_len		= BSWAP32(epb.captured_len);		// BUG_8B973D84(2) FIX_8B973D84(2) #Parse the capture length from the read data
			wblock->data.packet.packet_len		= BSWAP32(epb.packet_len);		// BUG_3E6EE6AF(2) FIX_3E6EE6AF(2) #Parse the packet length from the read data
		} else {
			...
			wblock->data.packet.cap_len		= epb.captured_len;			// BUG_8B973D84(3) FIX_8B973D84(3) #Parse the capture length from the read data (alternative)
			wblock->data.packet.packet_len		= epb.packet_len;			// BUG_3E6EE6AF(3) FIX_3E6EE6AF(3) #Parse the packet length from the read data (alternative)
		}
	} else {
		bytes_read = file_read(&pb, 1, sizeof pb, fh);						// BUG_8B973D84(4) FIX_8B973D84(4) BUG_3E6EE6AF(4) FIX_3E6EE6AF(4) #Read the header from the file (alternative)
		...
		if (pn->byte_swapped) {
			...
			wblock->data.packet.cap_len		= BSWAP32(pb.captured_len);		// BUG_8B973D84(5) FIX_8B973D84(5) #Parse the capture length from the read data
			wblock->data.packet.packet_len		= BSWAP32(pb.packet_len);		// BUG_3E6EE6AF(5) FIX_3E6EE6AF(5) #Parse the packet length from the read data
		} else {
			...
			wblock->data.packet.cap_len		= pb.captured_len;			// BUG_8B973D84(6) FIX_8B973D84(6) #Parse the capture length from the read data (alternative)
			wblock->data.packet.packet_len		= pb.packet_len;			// BUG_3E6EE6AF(6) FIX_3E6EE6AF(6) #Parse the packet length from the read data (alternative)
		}
	}
	...
	bytes_read = file_read((guchar *) (wblock->frame_buffer), 1, wblock->data.packet.cap_len, fh);	// BUG_8B973D84(7) FIX_8B973D84(8) BUG_3E6EE6AF(7) FIX_3E6EE6AF(8) #CWE-120 #Buffer overwrite when write data read from the file to the target buffer
...
pcapng_read_simple_packet_block(FILE_T fh, pcapng_block_header_t *bh, pcapng_t *pn, wtapng_block_t *wblock,int *err, gchar **err_info _U_)
{
	...
	errno = WTAP_ERR_CANT_READ;
	bytes_read = file_read(&spb, 1, sizeof spb, fh);						// BUG_3E6EE6AF(8) FIX_3E6EE6AF(9) #Read the header from the file (alternative)
	...
	if(pn->byte_swapped) {
		wblock->data.simple_packet.packet_len	= BSWAP32(spb.packet_len);			// BUG_3E6EE6AF(9) FIX_3E6EE6AF(10) #Parse the packet length from the read data
	} else {
		wblock->data.simple_packet.packet_len	= spb.packet_len;				// BUG_3E6EE6AF(10) FIX_3E6EE6AF(11) #Parse the packet length from the read data (alternative)
	}

	wblock->data.simple_packet.cap_len = bh->block_total_length					// BUG_8B973D84(8) FIX_8B973D84(9) #Calculate the capture length, which can be very large
	...
	errno = WTAP_ERR_CANT_READ;
	bytes_read = file_read((guchar *) (wblock->frame_buffer), 1, wblock->data.simple_packet.cap_len, fh);	// BUG_8B973D84(9) FIX_8B973D84(11) BUG_3E6EE6AF(11) FIX_3E6EE6AF(13) #CWE-120 #Buffer overwrite when write data read from the file to the target buffer
