 ...
 *
 * $Id: packet-tcp.c 28366 2009-05-14 23:33:17Z gerald $
 *
...
dissect_tcp(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree)
{
  ...
  tcph->th_seq = tvb_get_ntohl(tvb, offset + 4);					// BUG_8C32D803(1) FIX_8C32D803(1) #CWE-823 #Read the TCP sequence number directly from the packet, so it can be very large
  ...
  if(tcpd && (tcph->th_flags & TH_FIN)
    ...
    if(msp){
      fragment_data *ipfd_head;
      ipfd_head = fragment_add(tvb, offset, pinfo, msp->first_frame,			// BUG_8C32D803(2) FIX_8C32D803(2) #5 #Pass possibly large number "tcph->th_seq - msp->seq" as paramter "frag_offset" to function "fragment_add"
			tcp_fragment_table,
			tcph->th_seq - msp->seq,
			tcph->th_seglen,
			FALSE );
