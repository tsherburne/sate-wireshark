 ...
 *
 * $Id: packet-dnp.c 28356 2009-05-13 19:46:11Z etxrab $
 *
...
dnp3_al_process_object(tvbuff_t *tvb, packet_info *pinfo, int offset, proto_tree *robj_tree, gboolean header_only, guint16 *al_objtype)
{
  ...
  *al_objtype =
  ...
  /* Object Qualifier */
  al_objq = tvb_get_guint8(tvb, offset);			// BUG_D5F4E690(12) FIX_D5F4E690(12) #Pass malformed pointer "tvb" to function "tvb_get_guint8"
...
dissect_dnp3_al(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree)
{
  ...
  /* Process Data Object Details */
  while (offset <= (data_len-2))  {  /* 2 octet object code + CRC32 */
    offset = dnp3_al_process_object(tvb, pinfo, offset, robj_tree, FALSE, &obj_type);		// BUG_D5F4E690(11) FIX_D5F4E690(11) #Pass malformed "tvb" to function "dnp3_al_process_object"
  }
...
/* Make entries in Protocol column and Info column on summary display */
  ...
  dl_len = tvb_get_guint8(tvb, temp_offset);			// BUG_D5F4E690(1) FIX_D5F4E690(1) #Read "dl_len" from the packet
  temp_offset += 1;
      ...
      (dl_func != DL_FUNC_RESET_LINK) && (dl_func != DL_FUNC_ACK))
  {
    ...
    data_len = dl_len - 5;					// BUG_D5F4E690(2) FIX_D5F4E690(2) #Calculate "data_len" from tainted "dl_len"
    tmp = ep_alloc(data_len);					// BUG_D5F4E690(3) FIX_D5F4E690(3) #2 #Allocate "data_len" bytes for pointers "tmp" / "tmp_ptr"
    tmp_ptr = tmp;
    i = 0;
    data_offset = 1;  /* skip the transport layer byte when assembling chunks */	// BUG_D5F4E690(4) FIX_D5F4E690(4) #Initialize "data_offset" to 1
    while(data_len > 0)
    {
      guint8 chk_size;
      ...
      chk_ptr = tvb_get_ptr(tvb, offset, chk_size);		// BUG_D5F4E690(5) FIX_D5F4E690(5) #2 #Copy data from the packet into the allocated memory pointed at by "tmp_ptr"
    ...
    if (crc_OK)
    {
      al_tvb = tvb_new_child_real_data(tvb, tmp, (guint) (tmp_ptr-tmp), (gint) (tmp_ptr-tmp));		// BUG_D5F4E690(7) FIX_D5F4E690(7) #CWE-469 #Create a new TVB from the memory allocated in (3), but with incorrect ofssets calculated from corrupted pointer "tmp_ptr"
      ...
      if (! (tr_fir && tr_fin))
      {
        ...
        */
        ...
        next_tvb = process_reassembled_data(al_tvb, 0, pinfo,						// BUG_D5F4E690(8) FIX_D5F4E690(8) #3 #Build a new TVB from malformed "al_tvb"
      ...
      else
      {
        ...
        next_tvb = al_tvb;										// BUG_D5F4E690(9) FIX_D5F4E690(9) #Alternatively, set "next_tvb" to point to malformed "al_tvb"
    ...
    if (next_tvb)
    {
      al_result = dissect_dnp3_al(next_tvb, pinfo, dnp3_tree);						// BUG_D5F4E690(10) FIX_D5F4E690(10) #Pass malformed "next_tvb" to function "dissect_dnp3_al"
    }
  }
}
