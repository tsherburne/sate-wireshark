 ...
 *
 * $Id: packet-fcoe.c 27994 2009-04-08 13:07:21Z wmeier $
 *
...
dissect_fcoe(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree)
{
    gint crc_offset;
    gint eof_offset;
    gint frame_len = 0;												// BUG_220FD7EE(1) FIX_220FD7EE(1) #Declare variable "frame_len" as a signed integer
    ...
    if (tvb_get_guint8(tvb, 1)) {
        ...
        frame_len = tvb_reported_length_remaining(tvb, 0) - FCOE_HEADER_LEN - FCOE_TRAILER_LEN;			// BUG_220FD7EE(2) FIX_220FD7EE(2) #Calculate "frame_len" from packet data, so its value can be negative
    ...
    if (tree) {
        ...
        if (crc_exists) {
            ...
            crc_computed = crc32_802_tvb(next_tvb, frame_len);							// BUG_220FD7EE(3) FIX_220FD7EE(3) #CWE-190 #Convert potentially negative "frame_len" into an unsigned integer, causing it to overflow, before passing it to function "crc32_802_tvb"
            if (crc != crc_computed) {
                crc_msg = " [bad FC CRC]";
            }
        }
