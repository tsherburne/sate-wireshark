 ...
 *                   
 * $Id: packet-rtps.c 28729 2009-06-14 23:14:58Z gerald $
 *
...
static int rtps_util_add_bitmap(proto_tree *tree,
  ...
  gint32 num_bits;
  guint32 data;
  char temp_buff[MAX_BITMAP_SIZE];						// BUG_BFE3CCB1(1) FIX_BFE3CCB1(1) #CWE-121 #Allocate array "temp_buff" of size "MAX_BITMAP_SIZE" (200) on the stack
  int i, j, idx;
  proto_item * ti;
  ...
  num_bits = NEXT_guint32(tvb, offset, little_endian);				// BUG_BFE3CCB1(2) FIX_BFE3CCB1(2) #Read "num_bits" from a packet

  offset += 4;

  /* Reads the bits (and format the print buffer) */
  idx = 0;									// BUG_BFE3CCB1(3) FIX_BFE3CCB1(3) #Initialize index "idx" to zero
  for (i = 0; i < num_bits; i += 32) {						// BUG_BFE3CCB1(4) FIX_BFE3CCB1(4) #Loop up to "num_bits" using counter "i"
