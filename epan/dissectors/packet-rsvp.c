 ...
 *
 * $Id: packet-rsvp.c 27313 2009-01-27 21:46:58Z wmeier $
 *
...
rsvp_equal (gconstpointer k1, gconstpointer k2)
{
    ...
    if (ADDRESSES_EQUAL(&key1->source_info.source, &key2->source_info.source) == FALSE)	// BUG_916FD15B(5) FIX_916FD15B(6) #CWE-825 #CWE-416 #Later on, this function is called, after the ephemeral memory was freed. The macro calls function "memcmp" by passing a pointer to potentially expired memory, causing access to invalid memory
	return 0;
...
dissect_rsvp(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree)
{
    ...
    rsvph = ep_alloc(sizeof(rsvp_conversation_info));					// BUG_916FD15B(1) FIX_916FD15B(1) CWE-416 #Allocate ephemeral memory for pointer "rsvph"
    rsvph->session_type = 0;

    /* Copy over the source and destination addresses from the pinfo strucutre */
    SET_ADDRESS(&rsvph->source, pinfo->src.type, pinfo->src.len, pinfo->src.data);	// BUG_916FD15B(2) FIX_916FD15B(2) #Copy the address "pinfo->src.data" to "rsvph->source.data"
