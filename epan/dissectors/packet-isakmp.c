 ...
 *
 * $Id: packet-isakmp.c 28417 2009-05-19 20:00:05Z stig $
 *
  ...
  for (payload = initial_payload; length > 0; payload = next_payload) {
    ...
    if (payload_length >= 4) {	/* XXX = > 4? */
      ...
      if ((f = getpayload_func(payload, isakmp_version)) != NULL && f->func != NULL)
        (*f->func)(tvb, offset + 4, payload_length - 4, ntree, parent_tree,					// BUG_AD8F584C(6) FIX_AD8F584C(7) #CWE-824 #2 Calling function "dissect_enc", passing parameter "pinfo" with member "private_data" uninitialized
...
dissect_isakmp(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree)
{
  ...
  if (isakmp_version == 1) {
    ...
    tvb_memcpy(tvb, spii, offset, COOKIE_SIZE);									// BUG_AD8F584C(1) FIX_AD8F584C(2) #4 #Initialize "hash_key" with user-controlled data
    ...
    ike_sa_data = g_hash_table_lookup(ikev2_key_hash, &hash_key);						// BUG_AD8F584C(2) FIX_AD8F584C(3) #Looking up user-controlled "hash_key" in table "ikev2_key_hash", so it can fail
    if (ike_sa_data) {												// BUG_AD8F584C(3) FIX_AD8F584C(4) #Assuming the lookup failed, this branch is not executed
      ...
      pd_save = pinfo->private_data;
      pinfo->private_data = ikev2_dec_data;									// BUG_AD8F584C(4) FIX_AD8F584C(5) #CWE-824 #Since this branch is not executed, pointer "pinfo->private_data" is not initalized
      pd_changed = TRUE;
    }
  }
#endif /* HAVE_LIBGCRYPT */
  ...
  if (tree) {
	...
	if (decr) {
      ...
      dissect_payloads(tvb, isakmp_tree, tree, isakmp_version, hdr.next_payload,				// BUG_AD8F584C(5) FIX_AD8F584C(6) #CWE-824 #2 #Calling function "dissect_payloads" passing parameter "pinfo" with member "private_data" uninitialized
           offset, len, pinfo);
  }
#ifdef HAVE_LIBGCRYPT
  if (pd_changed) pinfo->private_data = pd_save;
    ...
    proto_tree *p _U_, packet_info *pinfo, int isakmp_version _U_, int unused _U_, guint8 inner_payload)
{
  ...
  if (pinfo->private_data) {
    key_info = (ikev2_decrypt_data_t*)(pinfo->private_data);							// BUG_AD8F584C(7) FIX_AD8F584C(8) #CWE-824 #Pointer "key_info" is initialized from "pinfo->private_data", which is unitialized
    encr_key_len = key_info->encr_spec->key_len;								// BUG_AD8F584C(8) FIX_AD8F584C(9) #CWE-824 #3 #Pointer "key_info" is invalid and dereferenced
