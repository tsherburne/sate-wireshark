 ...
 *
 * $Id: packet-dns.c 28052 2009-04-14 18:31:02Z wmeier $
 *
    ...
    const guchar **name)
{
  ...
  guchar *np;								// BUG_52CA9094(1) FIX_52CA9094(1) #CWE-824 #Declare pointer "np" without initializing it.
  int len = -1;
  ...
  for (;;) {
    ...
    case 0x00:
      ...
      if (np != *name) {
        ...
        if (maxname > 0) {
          *np++ = '.';							// BUG_52CA9094(2) FIX_52CA9094(3) #CWE-824 #Write to memory pointed to by "np", which might be uninitialized.
          maxname--;
        }
      }
      while (component_len > 0) {
        ...
        if (maxname > 0) {
          *np++ = tvb_get_guint8(tvb, offset);				// BUG_52CA9094(3) FIX_52CA9094(4) #CWE-824 #Write to memory pointed to by "np", which might be uninitialized.
          maxname--;
        }
      	component_len--;
      	offset++;
