 ...
 *
 * $Id: packet-rtps2.c 27994 2009-04-08 13:07:21Z wmeier $
 *
...
static int rtps_util_add_bitmap(proto_tree *tree,
  ...
  gint32 num_bits;
  guint32 data;
  char temp_buff[MAX_BITMAP_SIZE];						// BUG_D5800C2A(1) FIX_D5800C2A(1) #CWE-121 #Allocate array "temp_buff" of size "MAX_BITMAP_SIZE" (200) on the stack
  int i, j, idx;
  proto_item * ti;
  ...
  num_bits = NEXT_guint32(tvb, offset, little_endian);				// BUG_D5800C2A(2) FIX_D5800C2A(2) #Read "num_bits" from a packet

  offset += 4;

  /* Reads the bits (and format the print buffer) */
  idx = 0;									// BUG_D5800C2A(3) FIX_D5800C2A(3) #Initialize index "idx" to zero
  temp_buff[0] = '\0';
  for (i = 0; i < num_bits; i += 32) {						// BUG_D5800C2A(4) FIX_D5800C2A(4) #Loop up to "num_bits" using counter "i"
...
static int rtps_util_add_fragment_number_set(proto_tree *tree,
  ...
  gint32 num_bits;
  guint32 data;
  char temp_buff[MAX_BITMAP_SIZE];						// BUG_533440B6(1) FIX_533440B6(1) #CWE-121 #Allocate array "temp_buff" of size "MAX_BITMAP_SIZE" (200) on the stack
  int i, j, idx;
  proto_item * ti;
  proto_tree * bitmap_tree;
  const gint original_offset = offset;
   ...
   */
  num_bits = NEXT_guint32(tvb, offset+4, little_endian);			// BUG_533440B6(2) FIX_533440B6(2) #Read "num_bits" from a packet
  ...
  /* Reads the bits (and format the print buffer) */
  idx = 0;									// BUG_533440B6(3) FIX_533440B6(3) #Initialize index "idx" to zero
  temp_buff[0] = '\0';
  for (i = 0; i < num_bits; i += 32) {						// BUG_533440B6(4) FIX_533440B6(4) #Loop up to "num_bits" using counter "i"
    data = NEXT_guint32(tvb, offset, little_endian);
    offset += 4;
