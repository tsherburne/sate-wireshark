 ...
 *
 * $Id: packet-bacapp.c 27866 2009-03-27 16:17:31Z gerald $
 *
	...
	const value_string *src)
{
	...
	guint8 bf_arr[256];											// BUG_D206D57A(1) FIX_D206D57A(1) #CWE-457 #Declare buffer "bf_arr" without initialization
	proto_tree* subtree = tree;
	proto_item* ti = 0;
	...
	offset+=offs;
	unused = tvb_get_guint8(tvb, offset); /* get the unused Bits */						// BUG_D206D57A(2) FIX_D206D57A(3) #Read variable "unused" from the packet
	...
	for (i = 0; i < numberOfBytes; i++) {
		...
		if (i == numberOfBytes-1) { skip = unused; }							// BUG_D206D57A(3) FIX_D206D57A(4) #If variable "unused" is larger than 8
		for (j = 0; j < 8-skip; j++) {
			if (src != NULL) {
				if (tmp & (1 << (7 - j)))
				...
				bf_arr[min(255,(i*8)+j)] = tmp & (1 << (7 - j)) ? '1' : '0';			// BUG_D206D57A(4) FIX_D206D57A(5) #CWE-457 #Fill buffer "bf_arr"
			}
		}
	}
	...
	if (src == NULL)
	{
		bf_arr[min(255,numberOfBytes*8-unused)] = 0;							// BUG_D206D57A(5) FIX_D206D57A(6) #CWE-457 #Add a null-terminator to buffer "bf_arr", but if variable "unused" is too large, the terminator is added at offset 255, way beyond the end of the string the buffer contains, causing the middle of buffer "bf_arr" to remain uninitialized
		proto_tree_add_text(subtree, tvb, offset, lvt, "B'%s'", bf_arr);				// BUG_D206D57A(6) FIX_D206D57A(7) #CWE-457 #CWE-200 #Buffer "bf_arr" is added to the data tree for later display, without being fully initialized, leading to information leak
	}
	...
	case 10: /* buffer ready */
		while ((tvb_length_remaining(tvb, offset) > 0)&&(offset>lastoffset)) {
			...
			case 0: /* buffer-property */
				offset += fTagHeaderTree(tvb, subtree, offset, &tag_no, &tag_info, &lvt);
				offset = fDeviceObjectPropertyReference (tvb, subtree, offset);
				offset += fTagHeaderTree(tvb, subtree, offset, &tag_no, &tag_info, &lvt);
				break;
			case 1:
				offset = fUnsignedTag (tvb, subtree, offset,
...
static guint
fReadAccessSpecification (tvbuff_t *tvb, proto_tree *tree, guint offset)
{
	guint lastoffset = 0;
	guint8 tag_no, tag_info;
	guint32 lvt;
