 ...
 *
 * $Id: packet-dcp-etsi.c 28356 2009-05-13 19:46:11Z etxrab $
 *
...
void rs_deinterleave(const guint8 *input, guint8 *output, guint16 plen, guint32 fcount)
{
  ...
  for(fidx=0; fidx<fcount; fidx++)
  {
    ...
    for (r=0; r<plen; r++)
    {
      output[fidx+r*fcount] = input[fidx*plen+r];	// BUG_DE4059F0(4) FIX_DE4059F0(4) #CWE-680 #CWE-787 #Buffer "output" might be shorter than expected, leading to a potential overwrite 
    }
  }
}
 ...
 guint32 c_max, guint16 rsk, guint16 rsz _U_)
{
  ...
  for (i=0; i<c_max; i++)
  {
    ...
    index_coded += rsk;
    memcpy(output+index_out+PFT_RS_N_MAX, deinterleaved+index_coded, PFT_RS_P); // BUG_48FA9911(6) FIX_48FA9911(6) #CWE-122 #CWE-131 #CWE-680 #Copying data to potentially short buffer "output" can cause a buffer overwrite
...
)
{
  ...
  if(new_tvb) {								// BUG_48FA9911(3) #Don't check if "new_tvb" has enough data left to read
    ...
    decoded = rs_correct_data(deinterleaved, output, c_max, rsk, rsz);		// BUG_48FA9911(5) FIX_48FA9911(5) #Call function "rs_deinterleave", passing buffer "deinterleaved" as output
    if(tree)
...
)
{
  ...
  frag_edcp = fragment_add_seq_check (
    ...
    !last);
  if(fec) {
    new_tvb = dissect_pft_fec_detailed(				// BUG_85B4530F(3) FIX_85B4530F(4) #3 #Pass variable "plen" to function "dissect_pft_fec_detailed"
...
dissect_pft(tvbuff_t * tvb, packet_info * pinfo, proto_tree * tree)
{
  ...
  if (fcount > 1) {             /* fragmented*/
    ...
    guint16 real_len = tvb_length(tvb)-offset;			// BUG_85B4530F(1) FIX_85B4530F(1) #Variable "real_len" depends on the size of the current packet
