 ...
 *
 * $Id: packet-ipmi-transport.c 27983 2009-04-07 12:20:41Z wmeier $
 *
...
lan_24(tvbuff_t *tvb, proto_tree *tree)
{
	...
	for (i = 0; i < 8; i++) {										// FIX_1B24CC26(1) #CWE-834 #The loop condition keeps "i" into an acceptable range of values
		...
		s_tree = proto_item_add_subtree(ti, *ett[i]);							// BUG_1B24CC26(2) FIX_1B24CC26(2) #CWE-126 #Buffer "ett" of size 8 can be accessed out-of-bound if "i" is larger than 8
