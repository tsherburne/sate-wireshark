 ...
 *
 * $Id: packet-sigcomp.c 28356 2009-05-13 19:46:11Z etxrab $
 *
...
dissect_sigcomp_tcp(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree)
{
	...
	guint8		*buff;
	int			offset = 0;					// BUG_36CC316B(1) FIX_36CC316B(1) #Index "offset" is initialized to zero
	int			length;
	guint8		octet;
	guint16		data;
	int			i;
	...
	if(data == 0xffff){
		/* delimiter */
		offset = offset + 2;						// BUG_36CC316B(2) FIX_36CC316B(2) #Index "offset" is set at two
	...
	length = tvb_length_remaining(tvb,offset);				// BUG_36CC316B(3) FIX_36CC316B(3) #Assume the packet is of size three, therefore the remaining length is "3 - offset" equals one.
