 ...
 *
 * $Id: packet-nbns.c 27983 2009-04-07 12:20:41Z wmeier $
 *
...
{
	char *buf;											// BUG_4E27A311(1) FIX_4E27A311(1) #CWE-824 #Declare pointer "buf" without initializing it.
	guint16 opcode;
...
{
	struct tcpinfo	*tcpinfo = pinfo->private_data;			// BUG_419BD03E(7) FIX_419BD03E(9) #Initializing pointer "tcpinfo" with "pinfo->private_data", which can be NULL
	int		offset = 0;
	int		max_data;
	guint8		msg_type;
	guint8		flags;
	 ...
	 * into a message.)
	 */
	if (!tcpinfo->is_reassembled) {					// BUG_419BD03E(8) FIX_419BD03E(10) #CWE-476 #Dereferrencing pointer "tcpinfo", which can be NULL
		if (max_data < 4) {
			/*
