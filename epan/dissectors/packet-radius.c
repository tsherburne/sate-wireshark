 ...
 *
 * $Id: packet-radius.c 28729 2009-06-14 23:14:58Z gerald $
 *
...
static void dissect_attribute_value_pairs(proto_tree *tree, packet_info *pinfo, tvbuff_t *tvb, int offset, guint length) {
     ...
     * allocated (if any).
     */
    CLEANUP_PUSH(g_free, eap_buffer);						// BUG_79A7B3EC(1) FIX_79A7B3EC(1) #CWE-460 #Push cleanup callback for "eap_buffer"
    ...
    CLEANUP_CALL_AND_POP;							// FIX_79A7B3EC(6) #CWE-460 #The cleanup function is called and the stack restored properly
...
static void register_attrs(gpointer k _U_, gpointer v, gpointer p) {
	...
	int i;
	gint* ett = &(a->ett);
	gchar* abbrev = g_strconcat("radius.",a->name,NULL);			// BUG_28025470(4) FIX_28025470(1) #CWE-416 #Read from buffer "a->name", which has been freed if function "destroy_dict" has been called on a parsing error.
