 ...
 ** packet-netflow.c
 **
 ** $Id: packet-netflow.c 28488 2009-05-26 16:01:12Z wmeier $
 **
...
dissect_v9_flowset(tvbuff_t * tvb, packet_info * pinfo, proto_tree * pdutree, int offset, hdrinfo_t * hdrinfo)
{
	...
	if ((flowset_id == 0) || (flowset_id == 2)) {
		...
		proto_tree_add_item(pdutree, hf_cflow_flowset_length, tvb,
		    offset, 2, FALSE);
		offset += 2;
		dissect_v9_options(pdutree, tvb, offset, hdrinfo, flowset_id);
...
static int
dissect_v9_options(proto_tree * pdutree, tvbuff_t * tvb, int offset, hdrinfo_t * hdrinfo, guint16 flowset_id)
{
	...
	v9_template_add(&template);						// BUG_BE33920B(4) FIX_BE33920B(5) #Call sink function
