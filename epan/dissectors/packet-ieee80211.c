 ...
 *
 * $Id: packet-ieee80211.c 28662 2009-06-08 15:37:46Z gerald $
 *
  ...
  switch (tag_no)
  {
    ...
    case TAG_SSID:
      if(beacon_padding == 0) /* padding bug */
      {
        ...
        ssid = tvb_get_ephemeral_string(tvb, offset + 2, tag_len);		// BUG_4257033C(1) FIX_4257033C(1) #Read string "ssid" of variable length from the packet
#ifdef HAVE_AIRPDCAP
        AirPDcapSetLastSSID(&airpdcap_ctx, (CHAR *) ssid, tag_len);
#endif

	proto_tree_add_string (tree, tag_interpretation, tvb, offset + 2, tag_len, (char *) ssid);
