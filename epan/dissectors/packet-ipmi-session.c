 ...
 *
 * $Id: packet-ipmi-session.c 27324 2009-01-28 21:52:57Z wmeier $
 *
...
dissect_ipmi_session(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree)
{
	...
	if (authtype == IPMI_AUTH_RMCPP) {
		...
		if (authtype == IPMI_AUTH_NONE) {
			msg_start = 10;
			msg_len = tvb_get_guint8(tvb, 9);						// BUG_28B3BB4B(1) FIX_28B3BB4B(1) #Read the packet length from tainted data
		} else {
			msg_start = 26;
	...
	next_tvb = tvb_new_subset(tvb, msg_start, msg_len, -1);						// BUG_28B3BB4B(2) FIX_28B3BB4B(2) #Create a new container for the packet's data
	if (payloadtype_enc) {
