 ...
 *
 * $Id: sigcomp-udvm.c 28365 2009-05-14 20:12:43Z etxrab $
 *
...
execute_next_instruction:
	...
	if ( used_udvm_cycles > maximum_UDVM_cycles ){ // BUG_6553F682(1) #CWE-835 #Infinite loop here as counter "used_udvm_cycles" is not incremented on all paths
		result_code = 15;
	...
	case SIGCOMP_INSTR_INPUT_BYTES: /* 28 INPUT-BYTES (%length, %destination, @address) */
		...
		while ( n < length ){
			if (input_address > ( msg_end - 1)){
				current_address = at_address;
				result_code = 14;
				goto execute_next_instruction; // BUG_6553F682(24) FIX_6553F682(10) #CWE-835 #Infinite Loop here
			}
	...
	case SIGCOMP_INSTR_OUTPUT: /* 34 OUTPUT (%output_start, %output_length) */
		...
		while ( n < output_length ){
			...
			out_buff[output_address] = buff[k]; // BUG_7DD70701(2) FIX_7DD70701(2) #CWE-119 #Off-by-one overwrite of buffer "out_buff" of size 65535 when index "output_address" is 65535.
			string[0]= buff[k];
			string[1]= '\0';
			if (print_level_3 ){
				proto_tree_add_text(udvm_tree, bytecode_tvb, 0, -1,
