 ...
 *
 * $Id: packet.c 28662 2009-06-08 15:37:46Z gerald $
 *
	       ...
	       const guchar *pd, frame_data *fd, column_info *cinfo)
{
	...
	CATCH(ReportedBoundsError) {
		if(proto_malformed != -1){
			proto_tree_add_protocol_format(edt->tree, proto_malformed, edt->tvb, 0, 0,	// BUG_245F0BFF(8) FIX_245F0BFF(6) #CWE-754 #CWE-824 #Pointer "edt->tvb" might not be initialized and is passed to function "proto_tree_add_protocol_format", causing an invalid pointer dereference
    ...
    tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree)
{
	...
	if (dtbl_entry != NULL) {
		 ...
		 */
		saved_match_string = pinfo->match_string;												// BUG_848E3C4E(4) FIX_848E3C4E(5) #CWE-457 #CWE-824 #Pointer "pinfo" might not have been initialized on some paths, leading to an invalid pointer dereference
