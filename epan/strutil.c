 ...
 *
 * $Id: strutil.c 27999 2009-04-08 16:50:20Z gerald $
 *
...
bytes_to_str(const guint8 *bd, int bd_len) {
  return bytes_to_str_punct(bd,bd_len,'\0');		// BUG_B0954EED(4) FIX_B0954EED(5) BUG_256C7C53(3) FIX_256C7C53(3) #Pass potentially corrupted/uninitialized pointer "bd" to function "bytes_to_str_punct"
}
...
bytes_to_str_punct(const guint8 *bd, int bd_len, gchar punct) {
  ...
  while (bd_len > 0 && len > 0) {
    *p++ = hex[(*bd) >> 4];				// BUG_B0954EED(5) FIX_B0954EED(6) BUG_256C7C53(4) FIX_256C7C53(4) #2 #CWE-824 #CWE-125 #Access potentially corrupted/uninitialized pointer "bd", causing an out-of-bound read
    *p++ = hex[(*bd) & 0xF];
    len -= 2;
    bd++;
    bd_len--;
